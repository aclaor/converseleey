...
async function startRec(){
  finalTx = ""; chunks = []; t0 = Date.now(); running = true;
  ...
  if(SR){
    try{
      ...
      recog.onerror = ev => { if(ev.error === "not-allowed") $("#hint").textContent = "Mic blocked — type your answer instead."; };
      recog.onend = () => { if(running) try{ recog.start(); }catch(e){} };
      recog.start();
    }catch(e){}
  }
}
